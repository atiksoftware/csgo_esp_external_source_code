...
void CDraw::Line(float x1, float y1, float x2, float y2, float width, bool antialias, D3DCOLOR color)
{
	try{
		ID3DXLine *m_Line;
...
void CDraw::WeaponLogo(float x, float y,int weaponId, LPDIRECT3DDEVICE9EX dev){
	if(weaponId > 0 && weaponId < 100){
		if(weaponTried[weaponId] != true){
			std::stringstream ss;
			ss << "weapons/" << weaponId << ".png";
			std::string s = ss.str();
			std::wstring stemp = std::wstring(s.begin(), s.end());
			LPCWSTR sw = stemp.c_str();
			HRESULT aa = D3DXCreateTextureFromFile(dev, sw, &weaponTex[weaponId]);
			if(!FAILED(aa)){
				weaponSuccess[weaponId] = true;
				D3DXCreateSprite(dev, &weaponSprite[weaponId]);
			}
			weaponTried[weaponId] = true;
		}
		
		if(weaponSuccess[weaponId] == true){
			try{
				weaponSprite[42]->Begin(D3DXSPRITE_ALPHABLEND);
				weaponSprite[42]->Draw(weaponTex[weaponId], NULL, &D3DXVECTOR3(0, 8, 0), &D3DXVECTOR3(x, y, 0), 0xFFFFFFFF);
				weaponSprite[42]->End();
			} catch(...){
				cout << "error on CDraw::WeaponLogo - " << weaponId << endl;
			}
		}
	}
}
void CDraw::Sprite(LPDIRECT3DTEXTURE9 tex, float x, float y, float resolution, float scale, float rotation)
{
	try{
		...
		pDevice->SetPixelShader(NULL);
		sSprite->Begin(NULL);
		sSprite->SetTransform(&mat);
		sSprite->Draw(tex, NULL, NULL, NULL, 0xFFFFFFFF);
		sSprite->End();
	} catch(const std::exception&){
		cout << "error CDraw::Sprite" << endl;
	}
}
